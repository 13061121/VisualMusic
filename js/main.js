...
// 加载设置
    ...
    function setCurrentSong(num) {
        if (currentFile != num) {
            ...
            getAndShowCurrentSongInfo(fileList[currentFile]);
        }
    }

// 获得并显示当前歌曲信息

    function getAndShowCurrentSongInfo(file) {
        var t = $('#song-pic');
        if (typeof t[0].src == 'Blob')
            URL.resolveObjectURL(t[0].src);
        jsmediatags.read(file, {
                onSuccess: function (tag) {
                    if ('picture' in tag.tags) {
                        ...
                        t[0].src = "data:" + tag.tags.picture.format + ";base64," + window.btoa(base64String);
                    } else {
                        t[0].src = 'img/CDPicDefault.png';
                    }
                    showCurrentSongInfo(currentFile);
                },
...
// 显示当前歌曲信息
    function showCurrentSongInfo(num) {
        ...
        $fileList.find('li').each(function () {
            if (+$(this).attr('num') == num) {
                ...
                t.addClass('showAndHide');
                songNameTimer = setTimeout(function () {
                    songNameTimer = null;
                    $songName.removeClass('showAndHide');
                    t.removeClass('showAndHide');
                }, 15000);
                // 3秒后显示歌曲名
                /*setTimeout(function () {
                 $songName.animate({
